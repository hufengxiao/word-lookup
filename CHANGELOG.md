...
`run` 这类多义词展示混乱，根因是 `<span class="cf">` 在牛津里有**两种语义**被混为一种：

| <span class="cf"> 位置 | 真实含义 | 处理 |
|---|---|---|
| sense 义项顶、释义之前 | **义项短语头**（如 `run something` / `run on the dollar, pound, etc.`） | 显示在释义正上（已 v0.8.13） |
| 例句 `<li>` 内 | **例句的使用结构标注**（如 `+ adv./prep.`、`to run a marathon`） | 内联为句首灰字小标注，不单独成行 |

本次修复：
- **例句内的 `+ adv./prep.`、`+ adj.` 这类语法标注**：改为句首内联小标注（不再独立占一行刷屏）。
- **其余搭配型标注**（如 `take something with you`）仍单独一行再接例句。
- 修复顺带确定：**义项短语头一律紧跟其释义**，不再出现"写了个词组/标注后直接跳到下一个意思"。
- 真实 `run` 词条(verb+noun 双词性 43+义项)全链验证通过：所有义项短语头各跟其释义、例句分组、序号连续。

### 测试
- pytest(24) / ruff / 编译 全绿; 新增 1 条(例句 `+ adv.` 内联、短语头前置于释义)。
...
### 🆒 详情页：搭配短语与例句分行
上版把搭配短语(如 `take something with you`)和例句拼在同一行。本次把**搭配短语
单独一行**，换行后再接例句，再换行接中文翻译——三段式，仿纸质词典排版更清晰。

## [v0.8.11] - 2026-09-04

### 📖 详情页：例句紧跟所属释义（不再全堆到末尾）
之前例句被渲染成"整篇集中提取丢到文档末尾的 EXAMPLE 区"；更麻烦的是只取到
**第一个**样例块，像 abandon / take 这种十几条例句的词，绝大多数例句直接丢光。
本次基于真实牛津高阶第10版 HTML 结构改进：

- **例句按释义对齐**：每条释义正下方的 `<ul class=examples>` 里的例句，现在紧跟
  该释义输出（英文 + 中文 + 前面的搭配短语前缀，如 `take something with you`），
  把它放到提炼的语境里一起读。
- **例句零丢失**：交集 classifier 在「第 k 条释义」收集，所有 sense 各自例句都保留
  （含动词短语、名词等其它词性块）。
- **兼容兜底**：仅当整篇没有识别到任何 sense 内例句、却仍有整体例句时，才退化
  用旧的末尾 EXAMPLE 区（非常规结构词条保底），平时不再出现宿主尾部 EXAMPLE。

### 测试
- 编译 / ruff / pytest(22) 全绿；新增 2 条回归(例句归属 + 搭配前缀)。
- 用真实 oxford.db 的 abandon/take 渲染冒烟：例句皆紧跟释义、无集中 EXAMPLE。

## [v0.8.10] - 2026-09-04

### 🔁 单实例二次启动「唤起」窗口
此前程序已在系统托盘常驻时，再次双击 .exe 只会向日志打一行「已有实例在运行」便退出，
没有任何界面反馈，用户以为没启动成功。本次加**单实例唤醒**：

- 第一实例启动后在后台挂一个本地 IPC(具名管道 `wordlookup.wakeup.socket`)监听。
- 再次启动同一应用时，新进程检测到已在运行，**不再静默退出**，而是向管道发一条
  唤醒消息；首个实例收到后把搜索框 `show + 置顶 + 聚焦` 拉到前台——体验与
  Spotlight/常见常驻工具的「二次启动唤起」一致。
- 失败兜底：唤醒失败只记日志，不阻塞进程退出，不影响单实例锁原本的安全性。

触发途径：双击 exe / 快捷方式再次启动、命令行重复执行，均会把已驻留托盘的窗口唤到前台。

### 测试
- `py_compile` / ruff / pytest(20) 全绿；IPC 链路冒烟(实际 listen+connect+sent 并收)通过。
